    ...
    <>
      {!hasDecision ? (
        <section className="consentBanner" aria-labelledby="consent-title" aria-describedby="consent-description">
          <div><h2 id="consent-title">{isEn ? 'Your privacy choices' : 'As tuas escolhas de privacidade'}</h2><p id="consent-description"><span className="consentBanner__descriptionLong">{isEn ? 'Necessary storage keeps the service working. Analytics and advertising only start with your permission.' : 'O armazenamento necessário mantém o serviço a funcionar. Analytics e publicidade só começam com a tua autorização.'}</span><span className="consentBanner__descriptionShort">{isEn ? 'Essentials keep the service working; analytics and ads need your permission.' : 'O essencial mantém o serviço; analytics e publicidade precisam da tua autorização.'}</span></p>{privacySignal ? <p className="consentBanner__signal">{isEn ? 'Your browser privacy signal is active; advertising remains off.' : 'O sinal de privacidade do browser está ativo; a publicidade permanece desligada.'}</p> : null}</div>
